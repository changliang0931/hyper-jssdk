...
function doEncrypt(msg, publicKey, cipherMode = 1) {
    let cipher = new SM2Cipher();                           //每次加解密都要newCipher，记录环境
    msg = _.hexToArray(_.parseUtf8StringToHex(msg));
    //###
    console.log('msg')
    console.log(msg)
    if (publicKey.length > 128) {
      publicKey = publicKey.substr(publicKey.length - 128); //后128,去掉开头的04
    }
    let xHex = publicKey.substr(0, 64);
    let yHex = publicKey.substr(64);
    publicKey = cipher.createPoint(xHex, yHex);     //解析得到公钥的点

    let c1 = cipher.initEncipher(publicKey);        //计算c1,p2，调用reset
    cipher.encryptBlock(msg);                       //计算c2
    let c2 = _.arrayToHex(msg);                     

    let c3 = new Array(32);
    cipher.doFinal(c3)                              //计算c3
    c3 = _.arrayToHex(c3);
...
function doDecrypt(encryptData, privateKey, cipherMode = 1) {
    ...
    cipher.initDecipher(privateKey, c1);            //计算p2 = c1^priv
